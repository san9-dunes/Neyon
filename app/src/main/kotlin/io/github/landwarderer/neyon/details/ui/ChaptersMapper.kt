...
import io.github.landwarderer.neyon.details.ui.model.toListItem
...
fun MangaDetails.mapChapters(
	...
	downloadStates: Map<Long, Pair<Float, Boolean>> = emptyMap(),
	...
	if (!isDownloadedOnly || local?.manga?.chapters == null) {
		for (chapter in remoteChapters) {
			...
			result += (local ?: chapter).toListItem(
				...
				isGrid = isGrid,
				downloadPercent = downloadStates[chapter.id]?.first,
				isDownloadPaused = downloadStates[chapter.id]?.second ?: false,
			)
		}
	}
