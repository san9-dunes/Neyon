...
abstract class MangaDao {
	...
	@Upsert
	abstract suspend fun upsertAll(mangaList: Collection<MangaEntity>)
	...
	@Transaction
	open suspend fun upsertAllWithTags(mangaList: Collection<MangaEntity>, tagsMap: Map<Long, Iterable<TagEntity>>) {
		upsertAll(mangaList)
		val tagsToInsert = mutableListOf<MangaTagsEntity>()
		val idsToClear = mutableListOf<Long>()
		mangaList.forEach { manga ->
			val tags = tagsMap[manga.id]
			if (tags != null) {
				idsToClear.add(manga.id)
				tags.forEach { tag ->
					tagsToInsert.add(MangaTagsEntity(manga.id, tag.id))
				}
			}
		}
		if (idsToClear.isNotEmpty()) {
			clearTagRelations(idsToClear)
		}
		if (tagsToInsert.isNotEmpty()) {
			insertTagRelations(tagsToInsert)
		}
	}
}
