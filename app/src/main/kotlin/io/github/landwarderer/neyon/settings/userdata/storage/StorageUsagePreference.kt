...
import io.github.landwarderer.neyon.core.ui.widgets.SegmentedBarView
import io.github.landwarderer.neyon.core.util.FileSize
import io.github.landwarderer.neyon.core.util.NeyonColors
import io.github.landwarderer.neyon.databinding.PreferenceMemoryUsageBinding
...
class StorageUsagePreference @JvmOverloads constructor(
	...
	override fun onBindViewHolder(holder: PreferenceViewHolder) {
		...
		val storageSegment = SegmentedBarView.Segment(
			...
			NeyonColors.segmentColorRandom(context, Color.BLUE),
		)
		val pagesSegment = SegmentedBarView.Segment(
			usage?.pagesCache?.percent ?: 0f,
			NeyonColors.segmentColorRandom(context, Color.GREEN),
		)
		val otherSegment = SegmentedBarView.Segment(
			usage?.otherCache?.percent ?: 0f,
			NeyonColors.segmentColorRandom(context, Color.GRAY),
		)
