...
abstract class FavouritesDao : MangaQueryBuilder.ConditionCallback {
	...
	abstract suspend fun findAllMangaIds(): LongArray
	...
	@Query("SELECT DISTINCT manga_id FROM favourites WHERE manga_id IN (:mangaIds) AND deleted_at = 0")
	abstract suspend fun findFavoriteIds(mangaIds: Collection<Long>): List<Long>
	...
	@Upsert
	abstract suspend fun upsertAll(entities: Iterable<FavouriteEntity>)
