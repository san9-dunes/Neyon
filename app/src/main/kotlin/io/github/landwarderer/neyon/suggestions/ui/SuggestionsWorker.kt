...
import io.github.landwarderer.neyon.core.util.ext.flatten
...
import io.github.landwarderer.neyon.core.util.ext.sanitize
import io.github.landwarderer.neyon.core.util.ext.takeMostFrequent
import io.github.landwarderer.neyon.core.util.ext.toBitmapOrNull
import io.github.landwarderer.neyon.core.util.ext.trySetForeground
...
import io.github.landwarderer.neyon.suggestions.domain.SuggestionRepository
...
class SuggestionsWorker @AssistedInject constructor(
	...
	private suspend fun doWorkImpl(): Int {
		...
		val producer = channelFlow {
			val skipNsfw = appSettings.isSuggestionsExcludeNsfw || appSettings.isNsfwContentDisabled
			val skipSfw = appSettings.isSfwContentDisabled
			for (it in sources) {
				if ((it.isNsfw() && skipNsfw) || (it.isSfw() && skipSfw)) {
					continue
				}
				launch {
...
filter = MangaListFilter(tags = setOfNotNull(tag), tagsExclude = excludedTags),
		...
		if (appSettings.isSfwContentDisabled) {
			list.removeAll { it.isSfw() }
		}
	...
	private companion object {
		...
		const val GROUP_SUGGESTION = "io.github.landwarderer.neyon.SUGGESTIONS"
